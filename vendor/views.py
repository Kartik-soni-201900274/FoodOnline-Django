...
def edit_category(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            category.vendor = get_vendor(request)
            category.slug = slugify(category.name) + "-" + str(category.id)
            form.save()
...
def add_food(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            form.save() # here saving only the form will work because we are adding food.vendor / food.slug to the food object returned by form so when we save form the food object is also saved
...
def edit_food(request, pk):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            food = form.save(
                commit=False
            )  # This is the same FoodItem instance that was initially retrieved, but now it has been updated with the data from the form.
            food.vendor = get_vendor(request)
            food.slug = slugify(foodtitle)
            form.save()
